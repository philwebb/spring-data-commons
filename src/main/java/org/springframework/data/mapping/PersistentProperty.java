...
public interface PersistentProperty<P extends PersistentProperty<P>> {
	...
	/**
	 * Returns the {@link TypeInformation} if the property references a
	 * {@link PersistentEntity}. Will return {@literal null} in case it refers to a simple
	 * type. Will return {@link Collection}'s component type or the {@link Map}'s value
	 * type transparently.
	 * @return
	 */
	...
	/**
	 * Returns the getter method to access the property value if available. Might return
	 * {@literal null} in case there is no getter method with a return type assignable to
	 * the actual property's type.
	...
	/**
	 * Returns the setter method to set a property value. Might return {@literal null} in
	 * case there is no setter available.
	 * @return the setter method to set a property value if available, otherwise
	 * {@literal null}.
	 */
	@Nullable
	Method getSetter();
	...
	/**
	 * Returns the with {@link Method} to set a property value on a new object instance.
	 * Might return {@literal null} in case there is no with available.
	 * <p/>
	 * With {@link Method methods} are property-bound instance {@link Method methods} that
	 * accept a single argument of the property type creating a new object instance.
	 *
	 * <pre class="code">
	 * class Person {
	 * 	final String id;
	 ...
	 * }
	 * </pre>
	 * @return the with {@link Method} to set a property value on a new object instance if
	 * available, otherwise {@literal null}.
	 * @since 2.1
	 */
	@Nullable
	Method getWither();
	...
	/**
	 * Returns whether the type of the {@link PersistentProperty} is actually to be
	 * regarded as {@link PersistentEntity} in turn.
	 * @return {@literal true} a {@link PersistentEntity}.
	 */
	boolean isEntity();

	/**
	 * Returns whether the property is a <em>potential</em> identifier property of the
	 * owning {@link PersistentEntity}. This method is mainly used by
	 * {@link PersistentEntity} implementation to discover id property candidates on
	 * {@link PersistentEntity} creation you should rather call
	 * {@link PersistentEntity#isIdProperty(PersistentProperty)} to determine whether the
	 * current property is the id property of that {@link PersistentEntity} under
	 * consideration.
	...
	/**
	 * Returns whether the current property is a <em>potential</em> version property of
	 * the owning {@link PersistentEntity}. This method is mainly used by
	 * {@link PersistentEntity} implementation to discover version property candidates on
	 * {@link PersistentEntity} creation you should rather call
	 * {@link PersistentEntity#isVersionProperty(PersistentProperty)} to determine whether
	 * the current property is the version property of that {@link PersistentEntity} under
	 * consideration.
	 * @return
	 */
	...
	/**
	 * Returns the component type of the type if it is a {@link java.util.Collection}.
	 * Will return the type of the key if the property is a {@link java.util.Map}.
	 * @return the component type, the map's key type or {@literal null} if neither
	 * {@link java.util.Collection} nor {@link java.util.Map}.
	 */
	@Nullable
	...
	/**
	 * Returns the actual type of the property. This will be the original property type if
	 * no generics were used, the component type for collection-like types and arrays as
	 * well as the value type for map properties.
	 * @return
	 */
	Class<?> getActualType();

	/**
	 * Looks up the annotation of the given type on the {@link PersistentProperty}. Will
	 * inspect accessors and the potentially backing field and traverse accessor methods
	 * to potentially available super types.
	...
	/**
	 * Looks up the annotation of the given type on the property and the owning type if no
	 * annotation can be found on it. Useful to lookup annotations that can be configured
	 * on the type but overridden on an individual property.
	...
	/**
	 * Returns whether property access shall be used for reading the property value. This
	 * means it will use the getter instead of field access.
	 * @return
	 */
	 ...
	 * @return the type the property refers to in case it's an association or
	 * {@literal null} in case it's not an association, the target entity type is not
	 * explicitly defined (either explicitly or through the property type itself).
	 * @since 2.1
	 */
	@Nullable
	...
	/**
	 * Returns a {@link PersistentPropertyAccessor} for the current property's owning
	 * value.
